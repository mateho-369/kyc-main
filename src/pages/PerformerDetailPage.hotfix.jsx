...
const PerformerDetailPage = () => {
 ...
 // 画像URLを直接生成（緊急修正版 - パス修正済み）
 const fetchImageAsBlob = async (documentType) => {
   try {
     console.log('🖼️ 画像URL生成開始:', documentType);
     setImageLoadError(false);
     
     // 直接画像URLを生成（/uploads/performers/パスを使用）
     const baseUrl = 'https://stg.id-manager.com';
     // パフォーマーIDとドキュメントタイプから画像ファイル名を構築
     const imageUrl = `${baseUrl}/uploads/performers/${id}/${documentType}.jpg`;
              ...
              {imageLoadError ? (
                ...
                <img
                  ...
                  onError={(e) => {
                    ...
                    setImageLoadError(true);
                    // フォールバック: 複数のパスパターンを試す
                    if (!e.target.dataset.retried) {
                      e.target.dataset.retried = '1';
                      // パターン1: /uploads/performers/{id}_{type}.jpg
                      e.target.src = `https://stg.id-manager.com/uploads/performers/${id}_${previewDoc.type}.jpg`;
                    } else if (e.target.dataset.retried === '1') {
                      e.target.dataset.retried = '2';
                      // パターン2: /uploads/{id}/{type}.jpg
                      e.target.src = `https://stg.id-manager.com/uploads/${id}/${previewDoc.type}.jpg`;
                    } else if (e.target.dataset.retried === '2') {
                      e.target.dataset.retried = '3';
                      // パターン3: /uploads/{type}_{id}.jpg
                      e.target.src = `https://stg.id-manager.com/uploads/${previewDoc.type}_${id}.jpg`;
                    }
                  }}
                />
              ) : (
