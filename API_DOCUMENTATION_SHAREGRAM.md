...
**Mandatory owner scope (security):** List requests without an owner scope return `400` (`owner scope required: user_id / external_ids`); the API never returns a global performer list. Use `GET /api/performers?user_id=<Firebase uid>` or `GET /api/sharegram/performers?user_id=<Sharegram account id>`. On `/api/performers`, a matching Firebase UID takes precedence; otherwise `user_id` is interpreted as a Sharegram account id. `external_ids` is an explicit scope on Sharegram list requests.
